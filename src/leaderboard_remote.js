...
const RemoteLeaderboard = {
    ...
    // Отправить результат через RPC submit_score: одна запись на игрока в режиме mode,
    // хранит лучший результат (очки, при равенстве — время).
    submit(name, score, time, mode, cb) {
        ...
        fetch(SUPABASE_URL + '/rest/v1/rpc/submit_score', {
            ...
            body: JSON.stringify({ p_name: name, p_score: score, p_time: time, p_mode: mode || 'normal' }),
        })
